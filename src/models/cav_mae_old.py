...
class CAVMAE(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        # self.log_var_mae = torch.nn.Parameter(torch.zeros(1, device = "cuda"), requires_grad=True)
        # self.log_var_c = torch.nn.Parameter(torch.zeros(1, device = "cuda"), requires_grad=True)
        # self.log_var_latent = torch.nn.Parameter(torch.zeros(1, device = "cuda"), requires_grad=True)
        # self.log_var_v = torch.nn.Parameter(torch.zeros(1, device = "cuda"), requires_grad=False)
        # self.log_var_a = torch.nn.Parameter(torch.zeros(1, device = "cuda"), requires_grad=False)
        # self.log_var_distill_a = torch.nn.Parameter(torch.zeros(1, device="cuda"), requires_grad=False)
        # self.log_var_distill_v = torch.nn.Parameter(torch.zeros(1, device="cuda"), requires_grad=False)
        # self.log_var_encoder_distill = torch.nn.Parameter(torch.zeros(1, device="cuda"), requires_grad=False)
        # self.log_var_classification = torch.nn.Parameter(torch.zeros(1, device="cuda"), requires_grad=True)
        ...
        self.audio_classifier = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(intermediate_dim, 319)
            )
        ...
        # self.layer_after_joint_decoder = nn.Sequential(
        #             nn.Linear(decoder_embed_dim, 128),
        #             nn.ReLU(),
        #             nn.Linear(128, 5)
        #         )
    ...
    def forward_encoder(self, a, v, mask_ratio_a, mask_ratio_v, mask_mode='unstructured'):
        ...
        for blk in self.blocks_v:
           v = blk(v)
        """     
        student_a = [blk(a) for blk in self.blocks_a]
        student_v = [blk(v) for blk in self.blocks_v]

        teacher_a = [blk(a) for blk in self.blocks_a]
        teacher_v = [blk(v) for blk in self.blocks_v]

        # Centering teacher's outputs & applying temperature-scaled softmax
        #last_output_teacher_a = teacher_a[-1]
       # last_output_teacher_v = teacher_v[-1]
        
        #print(teacher_a[-1].shape)
        #last_output_teacher_a -= last_output_teacher_a.mean(dim=0)
        #last_output_teacher_v -= last_output_teacher_v.mean(dim=0)

        tps = 0.1
        # student_a_last = student_a[-1]
        # student_v_last = student_v[-1]
        student_a = F.softmax(student_a[-1] / tps, dim=2)
        student_v = F.softmax(student_v[-1] / tps, dim=2)
        
        tpt = 0.9
        teacher_a = F.softmax(teacher_a[-1] / tpt, dim=2)
        teacher_v = F.softmax(teacher_v[-1] / tpt, dim=2)
        
        teacher_a_pooled = teacher_a.mean(dim=1)
        teacher_v_pooled = teacher_v.mean(dim=1)
        student_a_pooled = student_a.mean(dim=1)
        student_v_pooled = student_v.mean(dim=1)
        
        # Compute the distillation loss
        epsilon = 1e-8
        #distillation_loss_audio = - (teacher_v_pooled * torch.log(student_a_pooled+epsilon)).sum(dim=1).mean()
        #distillation_loss_video = - (teacher_a_pooled * torch.log(student_v_pooled+epsilon)).sum(dim=1).mean()
        distillation_loss_audio = F.kl_div(torch.log(student_a_pooled+epsilon), teacher_v_pooled, reduction='batchmean')
        distillation_loss_video = F.kl_div(torch.log(student_v_pooled+epsilon), teacher_a_pooled, reduction='batchmean')
        print("distillation_loss_audio and video")
        print(distillation_loss_audio)
        print(distillation_loss_video)
        weight_distill_a = 1 / (2 * torch.exp(self.log_var_distill_a))
        weight_distill_v = 1 / (2 * torch.exp(self.log_var_distill_v))
        distillation_loss = weight_distill_a * distillation_loss_audio + self.log_var_distill_a  + weight_distill_v * distillation_loss_video + self.log_var_distill_v
        print("Distillation Loss")
        print(distillation_loss)
        print(weight_distill_a.item(), weight_distill_v.item())
        """
    ...
    def forward_mae_loss(self, input, pred, mask, modality):
        ...
        loss = (pred - target) ** 2
        #mmd_loss = compute_mmd(latent_c_a, latent_c_v)
        loss = loss.mean(dim=-1)  # [N, L], mean loss per patch

        loss = (loss * mask).sum() / mask.sum()  # mean loss on removed patches
        return loss

    def forward(self, audio, imgs, mask_ratio_a=0.75, mask_ratio_v=0.75, mae_loss_weight=1., contrast_loss_weight=0.01, kd_loss_weight=0.1, mask_mode='unstructured', weighting_method = 'uncertainty', k=0.5, labels=None):
    # latent is used for reconstruction (mae), latent_c_{a,v} are used for contrastive learning
        ...
        #audio_logits = torch.nn.Softmax()(self.audio_classifier(latent_c_a.mean(dim=1)))
        #video_logits = torch.nn.Softmax()(self.video_classifier(latent_c_v.mean(dim=1)))
        audio_logits = self.audio_classifier(latent_c_a.mean(dim=1))
        video_logits = self.video_classifier(latent_c_v.mean(dim=1))
        if labels is None:
            raise ValueError("Labels are required for classification loss.")
        print("LABELS", labels)
        print("Audio Video logits", audio_logits.shape, video_logits.shape)
        audio_classification_loss = nn.CrossEntropyLoss()(audio_logits, labels)
        video_classification_loss = nn.CrossEntropyLoss()(video_logits, labels)
        
        # Apply softmax to logits
        audio_probs = torch.nn.functional.softmax(audio_logits, dim=1)
        video_probs = torch.nn.functional.softmax(video_logits, dim=1)
        loss_latent = (video_probs - audio_probs)**2
        print("MSE LATENT:", loss_latent)
        #print(latent_c_a.shape)
        #print(latent_c_v.shape)


        weights = torch.arange(1, 769).float()  # .float() is used to convert the tensor to float type
        #loss_latent = torch.mean((weights * 0.1 * latent_c_a.mean(dim=1) - latent_c_v.mean(dim=1))**2)  # Example: MSE or Euclidean distance
        #loss_latent = torch.mean((latent_c_a.mean(dim=1) - latent_c_v.mean(dim=1))**2)
        # 1. Compute the Cosine Similarity
        #cosine_sim = nn.CosineSimilarity(dim=1, eps=1e-6)
        #similarity = cosine_sim(latent_c_a.mean(dim=1), latent_c_v.mean(dim=1))
        
        # 2. Define the Cosine Loss
        #cosine_loss = 1 - similarity.mean()
        #loss_latent = cosine_loss*0.1
    ...
    # if contrastive loss is used
        ...
        #mmd_loss = self.compute_mmd(latent_c_a.mean(dim=1), latent_c_v.mean(dim=1))
            # Static Weighting (default method)
        if weighting_method == 'static':
            loss = loss_mae + loss_c
            # Normalized Dynamic Weighting
        elif weighting_method == 'normalized':
            epsilon = 1e-7
            normalized_weight_mae = self.dynamic_weight_mae * loss_mae.item() / (loss_mae.item() + loss_c.item() + epsilon)
            normalized_weight_contrastive = self.dynamic_weight_contrastive * loss_c.item() / (loss_mae.item() + loss_c.item() + epsilon)
            # Update dynamic weights (if needed)
            self.dynamic_weight_mae = 0.9 * self.dynamic_weight_mae + 0.1 * normalized_weight_mae
            self.dynamic_weight_contrastive = 0.9 * self.dynamic_weight_contrastive + 0.1 * normalized_weight_contrastive
            loss = normalized_weight_mae * loss_mae + normalized_weight_contrastive * loss_c
            
        # Uncertainty-based Dynamic Weighting
        elif weighting_method == 'uncertainty':
            # Introducing learnable log variances for each task. These should be initialized somewhere in your model's constructor.
            # log_var_mae = torch.nn.Parameter(torch.zeros(1, device=audio.device))
            # log_var_c = torch.nn.Parameter(torch.zeros(1, device=audio.device))
            """ 
            print("Variables") # printing
            print(self.log_var_mae)
            print(self.log_var_c)
            print(self.log_var_latent)
            log_var_mae_sigmoid = torch.sigmoid(self.log_var_mae)
            log_var_c_sigmoid = torch.sigmoid(self.log_var_c)
            log_var_classification_sigmoid = torch.sigmoid(self.log_var_classification)
            log_var_latent_sigmoid = torch.sigmoid(self.log_var_latent)
            weight_mae = 1 / (2 * torch.exp(log_var_mae_sigmoid))
            #weight_v = 1 / (2 * torch.exp(self.log_var_v))
            #weight_a = 1 / (2 * torch.exp(self.log_var_a))
            weight_c = 1 / (2 * torch.exp(log_var_c_sigmoid))
            weight_latent = 1 / (2 * torch.exp(log_var_latent_sigmoid))
            weight_classification = 1 / (2 * torch.exp(log_var_classification_sigmoid))
            #weight_encoder_distill = 1 /(2 * torch.exp(self.log_var_encoder_distill))
            #print("Weight Weights MAE, Contr, Latent")
            #print(weight_mae.item(), weight_c.item(), weight_latent.item())
            #loss_mae_v = weight_v * loss_mae_v + self.log_var_v
            #loss_mae_a = weight_a * loss_mae_a + self.log_var_a
            #loss_latent = weight_latent*loss_latent + self.log_var_latent
            #loss_c = weight_c*loss_c + self.log_var_c
            """
            """
            weight_mae = 1 / (torch.square(torch.sigmoid(self.noise_param_mae)))
            weight_c = 1 / (torch.square(torch.sigmoid(self.noise_param_c)))
            weight_latent = 1 / (torch.square(self.noise_param_latent))
            weight_classification = 1 / (torch.square(torch.sigmoid(self.noise_param_classification)))
            #loss = weight_mae * loss_mae + weight_c * loss_c + weight_latent * loss_latent + log_var_mae_sigmoid + log_var_c_sigmoid + log_var_latent_sigmoid
            """
            # loss = weight_mae * loss_mae + weight_c * loss_c + weight_latent * loss_latent + torch.log(torch.sigmoid(self.noise_param_mae)) + torch.log(torch.sigmoid(self.noise_param_c)) + torch.log(torch.sigmoid(self.noise_param_latent))
            loss = loss_mae + loss_c + loss_latent + audio_classification_loss + video_classification_loss
            # loss = weight_outputs[0]*loss_mae + weight_outputs[1]*loss_c + weight_outputs[2]*loss_latent + weight_outputs[3]*audio_classification_loss + weight_outputs[4]*video_classification_loss
            print("LOSS in cavmae.py: ", loss)
        else:
            raise ValueError(f"Invalid weighting method: {weighting_method}")
        #loss = loss_mae + loss_c

        return loss, loss_mae, loss_mae_a, loss_mae_v, loss_c, mask_a, mask_v, c_acc, loss_latent, audio_logits, video_logits, audio_classification_loss, video_classification_loss
