...
import models
import numpy as np
from traintest_cavmae_old import train
...
# Audio input size torch.Size([30, 1024, 128])
# # Video input size torch.Size([30, 3, 224, 224]
# dummy_a_input = torch.rand(2, 1024, 128).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
# dummy_v_input = torch.rand(2, 3, 224, 224).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
dummy_a_input = torch.rand(2, 1024, 128)
dummy_v_input = torch.rand(2, 3, 224, 224)
audio_model = models.CAVMAE(audio_length=1024, norm_pix_loss=True, modality_specific_depth=11, tr_pos=False)
...
#     sys.stdout = f
#     summary(audio_model, [(1024, 128), (3, 224, 224)])    # Restore stdout
#     sys.stdout = original_stdout
print("AUDIO MODEL PARAMS")
print(audio_model.blocks_a.parameters())
labels = torch.rand(2, 319)
audio_model(dummy_a_input, dummy_v_input, labels=labels)
