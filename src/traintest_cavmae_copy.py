...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, label_indices) in enumerate(train_loader):
            ...
            print("Train test logits", audio_logits.shape, video_logits.shape)
            audio_classification_loss = nn.CrossEntropyLoss()(audio_logits, audio_labels)
            video_classification_loss = nn.CrossEntropyLoss()(video_logits, video_labels) 
        
            k = 2.5
            if audio_classification_loss < video_classification_loss:
                 video_classification_loss = video_classification_loss*torch.exp(k*(video_classification_loss - audio_classification_loss)/video_classification_loss)
            elif audio_classification_loss > video_classification_loss:
                audio_classification_loss = audio_classification_loss*torch.exp(k*(audio_classification_loss - video_classification_loss)/audio_classification_loss)
            
            classification_loss = audio_classification_loss + video_classification_loss
            
            print("Audio Classification Loss: ", audio_classification_loss)
            print("Video Classification Loss: ", video_classification_loss)
            print("Classification Loss:", classification_loss)
            #weight_classification = 1 / (2*torch.exp(torch.sigmoid(audio_model.module.log_var_classification)))
            weight_classification = 1 / (torch.square(audio_model.module.noise_param_classification))
            print("weight_Classification: ", weight_classification)

            loss = loss + weight_classification*classification_loss*1 + torch.log(torch.sigmoid(audio_model.module.noise_param_classification))
            print("Loss TrainTest", loss)
            """
            optimizer.zero_grad()

            all_losses = torch.stack([loss_mae, loss_c, loss_latent, audio_classification_loss, video_classification_loss]) # Adjust as needed
            ...
            total_loss = torch.dot(weights, all_losses)
            scaler.scale(loss).backward() # Use the computed total_loss for backpropagation

            scaler.step(optimizer)
            scaler.update()
            """
            # 1. Determine the Teacher and Student based on loss_mae_a and loss_mae_v
            if loss_mae_a < loss_mae_v:
                teacher_loss = loss_mae_a
                student_loss = loss_mae_v
                teacher_parameters = audio_parameters  
                student_parameters = video_parameters
                flag = 0
            else:
                teacher_loss = loss_mae_v
                student_loss = loss_mae_a
                teacher_parameters = video_parameters
                student_parameters = audio_parameters
                flag = 1
            ...
            # If audio is Teacher
            if flag == 0:
                loss_mae_v = loss_mae_v * scaling_factor
            else: # If video is Teacher
                loss_mae_a = loss_mae_a * scaling_factor

            # Backward pass for the main losses
            audio_optimizer.zero_grad()
            scaler.scale(loss_mae_a+loss_c+loss_latent).backward(retain_graph=True)
            scaler.step(audio_optimizer)
            
            video_optimizer.zero_grad()
            scaler.scale(loss_mae_v+loss_c+loss_latent).backward(retain_graph=True)
            scaler.step(video_optimizer)
            joint_optimizer.zero_grad()
            scaler.scale((loss_mae_a + loss_mae_v)/2 + loss_c + loss_latent).backward()
            # 3. Scale the gradients of the student's parameters
            #for param in student_parameters:
             #   if param.grad is not None:
              #      param.grad.data *= scaling_factor

            scaler.step(joint_optimizer)
            #scaler.step(weighting_optimizer)
            scaler.update()
            """
