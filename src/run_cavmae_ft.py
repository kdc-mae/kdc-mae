...
parser.add_argument("--dataset", type=str, default="vggsound", help="the dataset used", choices=["audioset", "esc50", "speechcommands", "fsd50k", "vggsound", "epic", "k400", "vggCurated", "kinetics", "imagenet", "ssv2"])
...
parser.add_argument("--kld_mlp", type=ast.literal_eval, help="KLD MLP")
...
print("dynamic_weight_normalization_method ", args.dynamic_weight_normalization_method)
print("absolute_noise ", args.absolute_noise)
print("dynamic_weighting ", args.dynamic_weighting)
print("knowledge_distillation ", args.knowledge_distillation)
print("k_value ", args.k_value)
print("kld_mlp ", args.kld_mlp)

if args.bal == 'bal':
    print('balanced sampler is being used')
    if args.weight_file != None:
        print(f"Using weight file: {args.weight_file}")
        samples_weight = np.loadtxt(args.weight_file, delimiter=',')
    else:
        ...
        if args.dataset == 'vggsound':
            ...
            print(f"Using weight file: {weight_path_else}")
            samples_weight = np.loadtxt(weight_path_else, delimiter=',')
        elif args.dataset == 'vggCurated':
            weight_path_else = '/home/amukherjee/Static/vgg_curated_train_weight.csv'
            print(f"Using weight file: {weight_path_else}")
            samples_weight = np.loadtxt(weight_path_else, delimiter=',')
            ...
            weight_path_else = '/home/alpha/Static/ilsrvc_train_weight.csv'
            print(f"Using weight file: {weight_path_else}")
            samples_weight = np.loadtxt(weight_path_else, delimiter=',')
        elif args.dataset == 'ssv2':
            weight_path_else = '/scratch/abhijitdas/Datasets/ssv2/ssv2_train_weight.csv'
            print(f"Using weight file: {weight_path_else}")
            samples_weight = np.loadtxt(weight_path_else, delimiter=',')
    sampler = WeightedRandomSampler(samples_weight, len(samples_weight), replacement=True)

    if args.dataset == 'vggsound' or args.dataset == 'vggCurated':
        ...
        train_loader = torch.utils.data.DataLoader(
            dataloader.ImageNetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf),
            batch_size=args.batch_size, sampler=sampler, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    elif args.dataset == 'ssv2':
        train_loader = torch.utils.data.DataLoader(
            dataloader.SSv2Dataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf),
            batch_size=args.batch_size, sampler=sampler, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    ...
    if (args.dataset == 'vggsound' or args.dataset == 'vggCurated'):
        ...
        train_loader = torch.utils.data.DataLoader(
            dataloader.ImageNetDataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True
        )
    elif (args.dataset == 'ssv2'):
        train_loader = torch.utils.data.DataLoader(
            dataloader.SSv2Dataset(args.data_train, label_csv=args.label_csv, audio_conf=audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True
        )
...
if args.dataset == 'vggsound' or args.dataset == 'vggCurated':
    ...
    val_loader = torch.utils.data.DataLoader(
        dataloader.ImageNetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf),
        batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    if args.data_eval != None:
        eval_loader = torch.utils.data.DataLoader(
            dataloader.ImageNetDataset(args.data_eval, label_csv=args.label_csv, audio_conf=val_audio_conf),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
elif args.dataset == 'ssv2':
    val_loader = torch.utils.data.DataLoader(
        dataloader.SSv2Dataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf),
        batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=True)
    if args.data_eval != None:
        eval_loader = torch.utils.data.DataLoader(
            dataloader.SSv2Dataset(args.data_eval, label_csv=args.label_csv, audio_conf=val_audio_conf),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
...
if args.skip_frame_agg == True:
    ...
    if (args.dataset == 'vggCurated' or args.dataset == 'vggsound'):
        val_loader = torch.utils.data.DataLoader(
            dataloader.VGGSound(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    elif (args.dataset == 'audioset'):
        val_loader = torch.utils.data.DataLoader(
            dataloader.AudiosetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    elif (args.dataset == 'kinetics'):
        val_loader = torch.utils.data.DataLoader(
            dataloader.KineticsDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    elif (args.dataset == 'imagenet'):
        val_loader = torch.utils.data.DataLoader(
            dataloader.ImageNetDataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    elif (args.dataset == 'ssv2'):
        val_loader = torch.utils.data.DataLoader(
            dataloader.SSv2Dataset(args.data_val, label_csv=args.label_csv, audio_conf=val_audio_conf, four_tiles=args.four_tiling, restrict_randomness=args.restrict_randomness),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True)
    ...
    if args.metrics == 'acc':
        ...
        print("Target", target)
        print("Argmax", np.argmax(target, 1))
        print("Mean target",np.mean(target, 1))
