...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs + 1:
        ...
        for i, (a_input, v_input, labels, *masks) in enumerate(train_loader):

            if args.restrict_randomness:
                print('using restricted randomness')
            ...
            with autocast():
                audio_output, loss = audio_model(a_input, v_input, args.ftmode, audio_mask=audio_mask, video_mask=video_mask, labels=labels, loss_fn=loss_fn)
                loss = loss.sum()
            optimizer.zero_grad()
...
def validate(audio_model, val_loader, args, output_pred=False):
    ...
    with torch.no_grad():
        for i, (a_input, v_input, labels, *masks) in enumerate(val_loader):
            ...
            with autocast():
                audio_output, loss = audio_model(a_input, v_input, args.ftmode, audio_mask=audio_mask, video_mask=video_mask, labels=labels, loss_fn=args.loss_fn)
